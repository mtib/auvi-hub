...
def getOutputDirs():
	...
	for l in loc:
		...
		for ap in arepara:
			folders.append(folders[0]+"/"+str(l[0])+"/plot/"+str(ap[1]))
	return folders
...
def moveOutputToWeb(delete=True):
	...
	if delete:
		os.system("mv -f "+getAuViDir()+OUTPUT_DIR+"/* "+getAuViDir()+WEB_DIR+"/. >/dev/null 2>/dev/null")
	else:
		os.system("cp "+getAuViDir()+OUTPUT_DIR+"/* "+getAuViDir()+WEB_DIR+"/. >/dev/null 2>/dev/null")
