...
\newcommand{\jftree}{\input{jftree.tex}}
% Ende Eigene Befehle
...
\subsubsection{Wahl der Umgebung} % AuVi Entwicklung
...
\lstinputlisting[language=Python,firstline=12, lastline=40,firstnumber=12,caption={,,Mainfunktion'' von \vsr\ (auvi.py)}]{\pyvidir auvi.py}
\lstinputlisting[language=Python,caption={Wrapper für Abhandeln der Ortsdefinitionen (local.py)}]{\pyvidir local.py}
...
\lstinputlisting[language=C,caption={GrADS Routine zum erstellen allgemeiner Funktionsgraphen (line.gs)}]{\pyvidir grads/line.gs}
\lstinputlisting[language=C,caption={GrADS Routine zum erstellen allgemeiner Konturplots (plot.gs)}]{\pyvidir grads/plot.gs}
